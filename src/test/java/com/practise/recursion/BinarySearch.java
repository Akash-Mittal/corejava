...
import com.am.innovations.validations.FailSafeNonNull;
...
enum BINARY_SEARCH_STRATEGY {
    ...
    ITERATIVE {
        ...
        public Integer search(Integer[] data, Integer valueToSearch) {
            Integer index = -1;
            if (FailSafeNonNull.checkIfNotNullAndSizeEqualsTo.test(data, 0)) {
                Integer start = 0;
    ...
    RECURSIVE {
        ...
        public Integer search(Integer[] data, Integer valueToSearch) {
            if (FailSafeNonNull.checkIfNotNullAndSizeEqualsTo.test(data, 0)) {
                return binarySearchRecursive(data, valueToSearch, 0, data.length - 1);
            } else {
                return -1;
            }
        }
